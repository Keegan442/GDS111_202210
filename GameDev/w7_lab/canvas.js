...
//create a new image to eventually add the canvas (this is instead of using <img> in html doc)
...
//now, "write" the image to te canvas page! an event must occur for this to happen...
galaxy.onload = function() {
    ...
    //use .fillRect and .strokeRect once styled
    ...
    //first need to move the "drawing cursor" to the starting point (x,y) of the lone
    ctx.moveTo(0,0) //start position
    //next draw a line to the ending point (x,y) of a line
    ctx.lineTo(800,600) //end position

    /*
    ctx.strokeStyle = "black" //the outline color
    ctx.lineWidth = "1"
    */
    ctx.stroke() //draws a line!
    
    //DRAW AN OPPPOSITE LINE
    ctx.moveTo(800,0)
    ctx.lineTo(0,600)
    ...
    ctx.lineTo(700, 140) //draws line to this new point
    ctx.lineTo(675, 200)
    ctx.lineTo(625, 200)
    ctx.lineTo(600, 140)
